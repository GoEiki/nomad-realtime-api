...
const userUpdates = ref<RealtimeEvent[]>([]);
...
function testsend(){
  const client = clientRef.value;
  if(client){
    client.sendNomadEvent({ event: 'transfer.event',data:'' });
  }
}
...
function setClient() {
  ...
  client.on('realtime.event', (realtimeEvent: RealtimeEvent) => {
    ...
    if(realtimeEvent.event.type === 'user.update'){
      userUpdates.value.push(realtimeEvent);
    }
  });
  ...
  <div>
    <div class="container">
      ...
      <div class="message-log">
        <h3>userupdates</h3>
        <ul>
            <li v-for="(event, index) in userUpdates" :key="index">
